...
const EditTask = ({ task, taskList, setTaskList }) => {
    ...
    return (
        <>
            <button className="bg-blue-500 text-white font-semibold  py-1 px-3 rounded-lg text-sm uppercase" onClick={()=> setEditModal(true)} >Edit</button>
            {editModal ?(
                <>
                    <div className="flex items-center justify center overflow-x-hidden overflow-y-auto fixed inset-0 z-100" >
                        <div className=" w-9/12 max-w-lg bg-white rounded-lg shadow-md relative flex flex-col" >
                            <div className=" flex flex-row justify-between p-5 border-b border-slate-200 rounded-t">
                                ...
                                <button className="px-1 text-gray-500 float-right text-3xl leading-none font-semibold block" onClick={() => setEditModal(false)} >X</button>
                            </div>
                            <form className="px-6 pt-6 pb-4">
                                <div>
                                    ...
                                    <input className="w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-5 leading-tight focus:outline-none focus:bg-white"
                            ...
                            <div className="flex justify-end p-6 border-t border-slate-200 rounded-b">
                                <button className="bg-blue-500 text-white rounded hover:opacity-70 font-semibold uppercase text-sm px-6 py-3" onClick={handleEdit} >Update Task</button>
                            </div>
                        </div>
